...
The Components AI icon control has been verified with Components `3.1.260813`. It only targets the empty-conversation icon host; it does not disable AI Chat, remove the prompt tip, or depend on Components' internal JavaScript API. If Components is unavailable or changes that class, the rule has no effect.
