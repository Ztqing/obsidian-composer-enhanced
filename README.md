...
- A code block theme selector with Composer's current appearance as the default and a Shiki-powered One Dark Pro palette that uses the same token colors in Reading view, Live Preview, and Source mode.
...
The selected code block palette applies in Reading view, Live Preview, and Source mode. One Shiki tokenizer produces the One Dark Pro token colors for all three views; Reading view keeps Obsidian's existing rendered code block and copy button, while both editor modes add non-editing CodeMirror decorations over fenced code content. This preserves text selection, horizontal scrolling, source editing, and existing syntax wrappers without changing note text.

The bundled language set follows CodeSuite's focused baseline and adds HTTP request blocks: Python, MATLAB, JavaScript, TypeScript, Java, C, C++, C#, Rust, Go, Bash, shell script, HTML, HTTP, CSS, JSON, YAML, TOML, SQL, Markdown, LaTeX, R, Ruby, Lua, Swift, Kotlin, XML, Diff, Dockerfile, Makefile, PowerShell, GraphQL, Haskell, Scala, PHP, Perl, TSX, JSX, and INI. Common aliases are normalized. Fences without a language and unsupported languages fall back to plain text so their colors remain consistent across views. `mermaid`, `dataview`, `dataviewjs`, and `query` are passthrough languages and remain available to Obsidian or their owning plugins.
